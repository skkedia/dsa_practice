...
public class HeightOfBinaryTreeAfterSubTreeRemovalQueries {
	...
	public int[] treeQueries(TreeNode root, int[] queries) {
		...
		for (Integer query : queries) {
			...
			int maxHeight = topHeight[level][0] == height ? topHeight[level][1] : topHeight[level][0];
	...
	private int findHeight(TreeNode root, int level, int[] nodeLevel, int[] nodeHeight, int[][] topheight) {
		if (root == null)
			return 0;

		int height = 1 + Math.max(findHeight(root.left, level + 1, nodeLevel, nodeHeight, topheight),
				findHeight(root.right, level + 1, nodeLevel, nodeHeight, topheight));
		nodeLevel[root.val] = level;
		nodeHeight[root.val] = height;

		/*
		 * here we are storing the greatest element in the 0th index and the 2nd
		 * greatest in the 1st index
		 * 
		 * so we check if height is greater than 0th index element if yes then we first
		 * swap it to the 1st index and then set 0th index to height
		 */
